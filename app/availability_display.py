...
def timerange2blockrange(timerange):
  ...
  #TODO: when getting timeranges, fetch the previous day's post-midnight range
...
def availability_display(request):
  ...
  try: startdate = datetime.strptime(request.GET["date"], "%Y-%m-%d").date()
  except: startdate = date.today()
  ...
  try:
    ...
    try: building = Building.objects.get(id=buildid)
    except: building = None
  
  if building is None:
    bdl = (Building.objects.annotate(rk=Count('roomkind__room', distinct=True),
                                     rkc=Count('roomkind', distinct=True))
                           .filter(rkc__gt=0).order_by('-rk'))
    for b in bdl:
      b.name = "%s (%d kinds, %d rooms)" % (b.name, b.rkc, b.rk) # don't save!
    ctex['bd_list'] = bdl
  else:
    ...
    if roomid is None: #building view
      ...
      display_list = []
      for (room, avdict) in get_availabilities_many(room_list, [startdate]):
    ...
    if display_list: 
      ...
      for (idx, (name, l, weeklyr, fetchedrs)) in enumerate(display_list):
        for a in fetchedrs:
      ...
      ctex['avail_table'] = zip([i[0:2] for i in display_list], avail_data)
